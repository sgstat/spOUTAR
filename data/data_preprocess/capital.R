# Total Economy: Household Capital Expenditures
...
df1 = time_series_df_clean[-c(1,8,14,15,23,48,51,53), ]
# View(df1)
df1 <- df1 %>%
  drop_na()
...
capital1 = df2[1:(indx1-1), ] # T x p format
capital2 = df2[(indx2-1):(indx3-1), ]

results = rep(0, 2)
Y = t(rbind(capital1, capital2))
cat("\n", dim(Y))
# tm1 = tic()
...
foreach(k = 2:8,
        ...
        .export = c("OUTAR", "Y")) %dopar% {
          ...
          # Save the model
          saveRDS(model, paste0("capital_results/model", k, ".rds"))
        }
