...
time_series_df_clean <- time_series_df_clean %>%
  select(`Reference area`, all_of(sorted_time_cols))

# View cleaned and sorted multivariate time series
# View(time_series_df_clean)

df1 = time_series_df_clean[-c(7,11,23,32,39,46,52,53), ]
# View(df1)
df1 <- df1 %>%
  drop_na()
...
df2 = apply(log(df1), 2, diff)#, 2, diff)
# df2 = df2[1:87, ]
# indx = which(rownames(df2) == "2009-Q2")
# rownames(df2) = NULL

import1 = df2[1:(indx1-1), ] # T x p format
import2 = df2[(indx2-1):(indx3-1), ]

results = rep(0, 2)
Y = t(rbind(import1, import2))
cat("\n", dim(Y))
# tm1 = tic()
# model <- OUTAR(
#   Y = 10000 * Y[, 1:78],
...
foreach(k = 2:8,
        ...
        .export = c("OUTAR", "Y")) %dopar% {
          ...
          # Save the model
          dir.create(paste0("OECD results new/import_results"), showWarnings = FALSE, recursive = TRUE)
          saveRDS(model, paste0("OECD results new/import_results/model", k, ".rds"))
        }
...
legend(
  ...
  lty = 1,
  lwd = 2,
  title = "Shifts in Import Expenditure Conditional \nDependencies After the 2009 Recession",
  box.lwd = 0,
  inset = 0.001
)
